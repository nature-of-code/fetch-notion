...
function hasProperty(node, field) {
  const value =
    ...
    node.type === 'element' &&
    node.properties &&
    lib_own.call(node.properties, field) &&
...
;// CONCATENATED MODULE: ./src/notion-api.js



const notionSecret = lib_core.getInput('notion-secret', { required: true });
const notion = new src/* Client */.KU({ auth: notionSecret });
...
async function fetchPublishedPages(databaseId) {
  lib_core.info(`Fetching database: ${databaseId}`);
...
;// CONCATENATED MODULE: ./src/handlers/rich-text.js
...
;// CONCATENATED MODULE: ./src/handlers/quote.js



function quote(block, parent) {
...
;// CONCATENATED MODULE: ./src/handlers/list.js
...
;// CONCATENATED MODULE: ./src/handlers/image.js
...
;// CONCATENATED MODULE: ./src/handlers/callout.js
...
;// CONCATENATED MODULE: ./src/handlers/table.js



function table(block, parent) {
...
;// CONCATENATED MODULE: ./src/handlers/code.js
...
function equation(block, parent) {
...
;// CONCATENATED MODULE: ./src/handlers/column.js


function column(block, parent) {
  const node = h('div', []);
  parent.children.push(node);
...
function column_list(block, parent) {
...
;// CONCATENATED MODULE: ./src/handlers/bookmark.js



function bookmark(block, parent) {
...
;// CONCATENATED MODULE: ./src/handlers/index.js
...
const handlers = {
  ...
  paragraph: paragraph,
  quote: quote,
  numbered_list_item: list_list('ol'),
  bulleted_list_item: list_list('ul'),
  image: image_image,
  table: table,
  callout: callout,
  code: code,
...
;// CONCATENATED MODULE: ./src/hast-from-notion.js



function hast_from_notion_convert(blocks) {
  const tree = h('', []);
...
function fromNotion(blocks, title) {
  const content = hast_from_notion_convert(blocks);
...
;// CONCATENATED MODULE: ./src/import-examples.js
...
async function downloadAndSaveFile({ url, relativePath }) {
  const streamPipeline = (0,external_node_util_namespaceObject.promisify)(external_node_stream_namespaceObject.pipeline);
  ...
  await streamPipeline(
    response.body,
    (0,external_node_fs_namespaceObject.createWriteStream)(external_node_path_namespaceObject.join('.temp/', relativePath)),
  );
...
async function downloadExample({ url, relativeDir }) {
  ...
  async function saveFile({ node, relativeDir }) {
    if (node.fileType === 'folder') {
      ...
      // create the folder
      await external_node_fs_namespaceObject.promises.mkdir(external_node_path_namespaceObject.join('.temp/', relativeDir, folderName), {
        recursive: true,
      });
      ...
      await Promise.all(
        node.children.map(async (childId) => {
          ...
          await saveFile({
            node: childNode,
            relativeDir: external_node_path_namespaceObject.join(relativeDir, folderName),
          });
        }),
      );
    }
    ...
    if (node.fileType === 'file') {
      if (node.content) {
        ...
        await external_node_fs_namespaceObject.promises.writeFile(
          external_node_path_namespaceObject.join('.temp/', relativeDir, node.name),
          node.content,
        );
      } else {
        ...
        await downloadAndSaveFile({
          url: node.url,
          relativePath: external_node_path_namespaceObject.join(relativeDir, node.name),
        });
      }
    }
  }

  const exampleDir = external_node_path_namespaceObject.join(relativeDir, lodash_es_snakeCase(data.name));

  // return screenshot path if exist
  let screenshotPath = null;
  if (data.files.find((file) => file.name === 'screenshot.png')) {
    screenshotPath = external_node_path_namespaceObject.join(exampleDir, 'screenshot.png');
  }
...
async function importExamples({ hast, slug }) {
  ...
  const relativeDir = external_node_path_namespaceObject.join('examples/', slug);
  await Promise.all(
...
async function importImages({ hast, slug }) {
  ...
  const relativeDir = external_node_path_namespaceObject.join('images/', slug);
  await external_node_fs_namespaceObject.promises.mkdir(external_node_path_namespaceObject.join('.temp/', relativeDir), {
    recursive: true,
  });
...
;// CONCATENATED MODULE: ./src/internal-links.js
...
async function main() {
  try {
    const notionDatabaseId = lib_core.getInput('notion-database-id', {
      required: true,
    });

    await external_node_fs_namespaceObject.promises.mkdir('.temp', { recursive: true });
    ...
    const outputFolder = lib_core.getInput('output-folder');
...
async function savePage({ fileName, hast }) {
  ...
  lib_core.info(`Creating file: ${fileName}.html`);
  await external_node_fs_namespaceObject.promises.writeFile(`.temp/${fileName}.html`, toHtml(hast));
}
